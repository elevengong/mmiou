...
class model {
	 ...
	 * @return unknown_type
	 */
	final public function listinfotags($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array(), $data = '*') {
		$where = to_sqls($where);
		$this->number = $this->count($where);
		$page = max(intval($page), 1);
		$offset = $pagesize*($page-1);
	...
	final public function list_tags_thumb($tagdata){
		$tagdata_thumb = array();
		foreach ($tagdata as $key => $tag)
		{
			...
			//$where = to_sqls($where);
			//print_r($where);
			//$tagdata_thumb[$key] = $tag[contentid];
			if($tag['modelid'] == 3)
			{
				$thumb =  $this->select_thumb($where, 'thumb', "", '', '', '', 'v9_picture');
			}
			if($tag['modelid'] == 1)
			{
				$thumb =  $this->select_thumb($where, 'thumb', "", '', '', '', 'v9_news');
			}
			//print_r($thumb);exit;
			$tagdata[$key][thumb] = $thumb[0][thumb];
		}
		return $tagdata;
	}
	
	final public function select_thumb($where = '', $data = '*', $limit = '', $order = '', $group = '', $key='', $table) {
		if (is_array($where)) $where = $this->sqls($where);
	 ...
	 * @return unknown_type
	 */
	final public function list_tags($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array()) {
		$this->table_name = 'v9_tags_content';
		...
		//echo $page."<br>";
		//exit;
		//$this->tags_pages = "aaaaaaaaaaaaa";
		$this->tags_pages = tags_pages($tag, $this->number, $page, $pagesize, $urlrule, $array, $setpages);
		$array = array();
