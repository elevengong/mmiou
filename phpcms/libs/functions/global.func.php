...
function template($module = 'content', $template = 'index', $style = '') {
    ...
    if(defined('HH_MOBILE')){
        $style .= hh_conf('mobile_tpl_suffix','_m');
    }
	
	if(isset($_GET['_t'])){
		exit($style.'/'.$module.'/'.$template);
	}
 ...
 * @return 分页
 */
function tags_pages($tag,$num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			...
			if($p_page>=0)
			{$p_page = 1;}
			$multipage .= ' <a href="/tags-'.$tag.'-'.$p_page.'.html" class="a1">'.L('previous').'</a>';
			if($curr_page==1) {
				$multipage .= ' <span>1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= ' <a href="/tags-'.$tag.'/">1</a>..';
			} else {
				$multipage .= ' <a href="/tags-'.$tag.'/">1</a>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <a href="/tags-'.$tag.'-'.$i.'.html">'.$i.'</a>';
			} else {
		...
		if($curr_page<$pages) {
			...
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<a href="/tags-'.$tag.'-'.$pages.'.html">'.$pages.'</a> <a href="/tags-'.$tag.'-'.$n_page.'.html" class="a1">'.L('next').'</a>';
			} else {
				$multipage .= ' <a href="/tags-'.$tag.'-'.$pages.'.html">'.$pages.'</a> <a href="/tags-'.$tag.'-'.$n_page.'.html" class="a1">'.L('next').'</a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span>'.$pages.'</span> <a href="/tags-'.$tag.'-'.$curr_page.'.html" class="a1">'.L('next').'</a>';
		} else {
			$c_page = $curr_page+1;
			$multipage .= ' <a href="/tags-'.$tag.'-'.$pages.'.html">'.$pages.'</a> <a href="/tags-'.$tag.'-'.$c_page.'.html" class="a1">'.L('next').'</a>';
		}
	}
	return $multipage;
}
...
function wz_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			...
			if($p_page>=0)
			{$p_page = 1;}
			$multipage .= ' <a href="/tags_'.$p_page.'.html" class="a1">'.L('previous').'</a>';
			if($curr_page==1) {
				$multipage .= ' <span>1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= ' <a href="/tags_1.html">1</a>..';
			} else {
				$multipage .= ' <a href="/tags_1.html">1</a>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <a href="/tags_'.$i.'.html">'.$i.'</a>';
			} else {
				$multipage .= ' <span>'.$i.'</span>';
			}
		}
		if($curr_page<$pages) {
			$n_page = $curr_page+1;
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<a href="/tags_'.$pages.'.html">'.$pages.'</a> <a href="/tags_'.$n_page.'.html" class="a1">'.L('next').'</a>';
			} else {
				$multipage .= ' <a href="/tags_'.$pages.'.html">'.$pages.'</a> <a href="/tags_'.$n_page.'.html" class="a1">'.L('next').'</a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span>'.$pages.'</span> <a href="/tags_'.$curr_page.'.html" class="a1">'.L('next').'</a>';
		} else {
			$c_page = $curr_page+1;
			$multipage .= ' <a href="/tags_'.$pages.'.html">'.$pages.'</a> <a href="/tags_'.$c_page.'.html" class="a1">'.L('next').'</a>';
		}
	}
	return $multipage;
}
