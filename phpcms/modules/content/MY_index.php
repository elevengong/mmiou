...
class MY_index {
	...
	function __construct() {
		...
		$this->db_tag = pc_base::load_model('tags_model');
	}
	...
	public function init() {
		...
		//tags
		$hot_tags = $this->db_tag->listinfotags('','hits desc', 1, 25,'','','','','tag');
		//print_r($hot_tags);
	...
	public function show() {
		...
		if($pictureurls && !$content){
		    $content = '';
		    $n = 0;
		    //echo "aaa";

		    foreach($pictureurls as $v){
		        $n++;

		        if(defined('HH_MOBILE')){
		            if(count($pictureurls) == $n){
		                $url = $next_page[id] ? hh_m_show_url($next_page) : 'javascript:alert(\'最后一页\')';
                    }else{
                        $url = str_replace('.html','_'.($n+1).'.html',hh_m_show_url($rs));

		            }

                }else{
                    $url = $this->url->show($id, $n+1, $catid, $rs['inputtime']);

                    $url = count($pictureurls) == $n ? $next_page['url'] : $url[0];
                }
                //echo $url;


                $alt = $title.' 图'.$n;
		        $content .= ($n > 1 ? ('[page]'.$title.'('.$n.')[/page]') : '').'<a href="'.$url.'"><img alt="'.$alt.'" src="'.$v['url'].'" /></a>';
            }
        }
